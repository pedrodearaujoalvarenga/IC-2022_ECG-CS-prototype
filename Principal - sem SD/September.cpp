...
const int delay_BLE_ligado = 5000;     //Em September, esse valor não é alterável
const int delay_BLE_desligado = 10000; //Em September, esse valor não é alterável

const int clockSlower = 10; //10 20 40 80 160 240
const int clockTurbo = 240; //80 160 240

int delay_geracao_Dados = 3;     //September ainda permite o controle da frequência de geração de dados
boolean compressedSensing = false; //September permite alterações de Compressed-Sensing
int compressedRatio = 0; //September permite alterações de Compressed-Sensing
  ...
  std::string coletadodaCaracteristica = pCharacteristic->getValue(); //braço para ler o valor da característica enviado pelo app
   ...
   int h = 0;
	...
	if(valoresColetadosdaSequencia[1] == 0){
		//Compressed Sensing off 
		compressedSensing = false;
		canStart = true;
		return;	
	}
	if(valoresColetadosdaSequencia[1] == 1){
